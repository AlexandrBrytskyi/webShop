...
<div class="wrap">
    <div class="navigation-strip">
        <h4>${category.name} - > ${subcategory.name} -> ${parent.name}<i class="arrow"></i></h4>
        <div class="top-menu">
            <!-- start header menu -->
    ...
    <div style="width: 80%; margin: auto">
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${characteristicValues}" var="cv">
                <%--<c:out value="${characteristic}"/>--%>
                <tr>
                    <th scope="row">${cv.id}</th>
                    <td>${cv.charecteristic.name}</td>
                    <td>${cv.charecteristic.type}</td>
                    <td>${cv.value}</td>
                    <td>
                        <div class="btn_form">
                            <form style="width: 100%; margin: auto"
                                  action="${baseURL}shop/admin/characteristics/values/delete/${subcategory.id}/${parent.id}/${cv.id}"
                                  method="post">
        ...
        <form id="crete_characteristic_form"
              action="${baseURL}shop/admin/characteristics/values/create/${subcategoryId}/${parent.id}"
              method="post">
